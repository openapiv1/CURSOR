...
import { useCustomChat as useChat } from "@/lib/use-custom-chat";
...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        setIsInitializing(true);
        console.log("Inicjalizuję E2B desktop sandbox...");

        // Use the provided ID or create a new one
        const { streamUrl, id } = await getDesktopURL(sandboxId ?? undefined);

        console.log("Sandbox utworzony z ID:", id);
        setStreamUrl(streamUrl);
        setSandboxId(id);
        toast.success("Desktop sandbox gotowy!", {
          position: "top-center",
        });
      } catch (err) {
        console.error("Failed to initialize desktop:", err);
        toast.error("Błąd inicjalizacji E2B sandbox - sprawdź klucz API", {
          position: "top-center",
        });
      } finally {
        setIsInitializing(false);
  ...
  return (
    <div className="flex h-dvh relative">
      ...
      <div className="w-full hidden xl:flex h-full">
        ...
        <div className="w-[28%] flex flex-col border-r border-zinc-200">
          <div className="bg-white py-4 px-4 flex justify-between items-center">
            <div onClick={resetChat} className="cursor-pointer">
              <AISDKLogo />
            </div>
          </div>
          ...
          <div className="bg-white">
            <form onSubmit={handleSubmit} className="p-4">
              <Input
                ...
                input={input}
                isInitializing={isInitializing || !sandboxId}
                isLoading={isLoading}
                status={status}
                stop={stop}
              />
        ...
        <div className="flex-1 bg-black relative items-center justify-center">
          {streamUrl ? (
            <>
              <iframe
                src={streamUrl}
